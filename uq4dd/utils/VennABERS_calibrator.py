# Authors: 
# Generation of probablistic point estimates: "Comparison of Scaling Methods to Obtain Calibrated Probabilities of Activity for Protein-Ligand Predictions.'
# Lewis H. Mervin, Avid M. Afzal, Ola Engkvist, and Andreas Bender, Journal of Chemical Information and Modeling 2020 60 (10), 4546-4559, DOI: 10.1021/acs.jcim.0c00476
# 
# Implementation of Venn Abers based on the implementation by Paolo Toccaceli, Royal Holloway, Univ. of London.
# "Large-scale probabilistic prediction with and without validity guarantees" (2015). See https://github.com/ptocca/VennABERS  for details.
...
import uq4dd.utils.VennABERS as va
...
def _fit_calibrator_with_va(clf, predictions, y, classes, method, sample_weight=None):
    ...
    for class_idx, this_pred in zip(pos_class_indices, predictions.T):
        ...
        elif method == "vennabers":
            calibrator = _VennAbersCalibration()
        else:
            raise ValueError(
                f"'method' should be one of: 'sigmoid' or 'isotonic' or 'vennabers'. Got {method}."
            )
...
class CalibratedClassifierCVWithVA(CalibratedClassifierCV):
    ...
    def __init__(
        ...
        *,
        ...
        if hasattr(base_estimator, "num_workers"):
            self.n_jobs = 1
        else:
            self.n_jobs = n_jobs
        self.n_jobs = n_jobs
